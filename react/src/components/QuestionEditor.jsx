    ...
    <>
            ...
            <div className='flex gap-3 justify-between mb-3'>
                ...
                <div>
                    ...
                    <select value={model.type} onChange={onTypeChange} name="questionType" className='mt-1 block w-full rounded-mb border border-gray-300 bg-white py-2 px-3 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm' id="questionType">
            ...
            <div>{shouldHaveOptions() && (<div>
                    ...
                    {model.data.options.length === 0 &&  (<div className='text-xs text-gray-600 text-center py-3'>
                        You dont have options definied
                    </div>) }
                    ...
                    {model.data.options.length > 0 && <div>
                        {model.data.options.map((op, ind) => (
                            <div key={op.uuid} className='flex items-center mb-1'> 
                                ...
                                <input type="text" value={op.text} className='w-full rounded-sm py-1 px-2 text-xs border border-gray-300 focus:border-indigo-500' onInput={ev => {op.text = ev.target.value; setModel({...model})}} />

                                
                                <button onClick={ev => deleteOption(op)} type='button' className='w-6 rounded-full flex items-center justify-center border border-transparent transition-color hover:border-red-100'> 
                                <TrashIcon className='w-5 h-5 text-red-500' /> </button>
